...
## 2026-09-23 bounded planning-view checkpoint (canonical PR #12)
- **START HEAD:** `aed4b928972a7f255d61d6c3f6dee3d7209899e4`; it is the exact requested checkpoint. The checkout had no configured remote and was renamed locally to `codex/connect-environmental-artifacts-to-runtime` without resetting or merging.
- **IMPLEMENTED:** `PlanningWorldView.open` now requires the complete dependency set for its consumer pass, exact expected plan UUID/root pairs, a byte-for-byte matching durable manifest, and successful validation of every referenced page before exposing the view. Missing, stale, duplicate, unrelated, corrupt, or unsealed layers fail closed.
- **BOUNDED EVIDENCE:** A 400-entry synthetic FLORA view crosses 136 pages in four exact upstream layers with an LRU bound of two decoded pages per layer. Lookups preserve declared pass priority and fall back to the generation-free caller-provided base accessor.
- **LIMITATION:** This checkpoint supplies the bounded exact planning-view primitive and adversarial unit coverage, but production preview dispatch does not yet use it. SOILS/AQUATIC/STRUCTURES/FLORA/TREES/HABITATS/VALIDATION therefore remain `PARTIAL`; complete live planning/apply/rollback and zero-touch readiness remain unclaimed.
