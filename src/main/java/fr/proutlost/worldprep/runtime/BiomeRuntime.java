...
public final class BiomeRuntime {
    public static List<WorldPrepSavedData.SnapshotCell> inspectColumn(ServerLevel level,int quartX,int quartZ,String appliedBiome){LevelChunk chunk=chunk(level,quartX,quartZ);var cells=new ArrayList<WorldPrepSavedData.SnapshotCell>();for(int qy=minQy(level);qy<=maxQy(level);qy++){String current=id(chunk.getNoiseBiome(quartX,qy,quartZ));cells.add(new WorldPrepSavedData.SnapshotCell(quartX,qy,quartZ,current,appliedBiome));}return List.copyOf(cells);}
    ...
    @SuppressWarnings("unchecked") public static void applyOwnedCell(ServerLevel level,WorldPrepSavedData.SnapshotCell cell){LevelChunk chunk=chunk(level,cell.quartX(),cell.quartZ());String current=id(chunk.getNoiseBiome(cell.quartX(),cell.quartY(),cell.quartZ()));if(current.equals(cell.appliedBiome()))return;if(!current.equals(cell.beforeBiome()))throw conflict(cell.quartX(),cell.quartY(),cell.quartZ(),cell.beforeBiome(),cell.appliedBiome(),current);Holder<Biome> target=holder(level,cell.appliedBiome());int section=chunk.getSectionIndex(QuartPos.toBlock(cell.quartY()));((PalettedContainer<Holder<Biome>>)chunk.getSection(section).getBiomes()).set(QuartPos.quartLocal(cell.quartX()),QuartPos.quartLocal(cell.quartY()),QuartPos.quartLocal(cell.quartZ()),target);chunk.setUnsaved(true);}
