...
class EnvironmentalCompletionTest {
 ...
 @Test void completePipelineIsPagedBoundedAndDeterministic(@TempDir Path temp)throws Exception{var input=new EnvironmentalPipeline.Input("test:area","minecraft:overworld","test",PlanFingerprint.of("source").value(),PlanFingerprint.of("pack").value(),42);var all=new EnumMap<PassId,List<EnvironmentalPipeline.Entry>>(PassId.class);for(var pass:PassGraph.order())if(pass!=PassId.VALIDATION){var values=new ArrayList<EnvironmentalPipeline.Entry>();for(int i=0;i<37;i++)values.add(new EnvironmentalPipeline.Entry(i,i%5,-i,pass.name()));all.put(pass,values);}var a=new EnvironmentalPipeline().publish(temp.resolve("a"),input,all,7);var b=new EnvironmentalPipeline().publish(temp.resolve("b"),input,all,7);assertEquals(a.run().plans(),b.run().plans());assertEquals(a.run().rootFingerprint(),b.run().rootFingerprint());for(var manifest:a.manifests().values()){var metrics=PagedPlanStore.read(temp.resolve("a").resolve(manifest.planId().toString()),manifest,EnvironmentalPipeline.CODEC,x->{});assertEquals(1,metrics.maximumResidentPages());assertTrue(metrics.maximumResidentEntries()<=7);}}
